function [Model, AUC, Accuracy,err_count, run_time, mistakes, mistakes_idx, SVs, TMs, AUC_list, Acc_list] = nKOIL_FIFOC( id_train, id_test, Y, K, options  )
...
for t = 1:size(id_train,1),
    ...
    if y_t==+1,
        ...
        [Ap,Bp,flag,ridx] = nUpdateBudgetFIFOC(a(t),id,Ap,Bp,Np,K);
        if(flag==1)
            if(ridx~=-1)
                App(ridx)=a(t);
            else 
        ...
        [An,Bn,flag,ridx] = nUpdateBudgetFIFOC(a(t),id,An,Bn,Nn,K);
        if(flag==1)
            if(ridx~=-1)
                Ann(ridx)=a(t);
            else 
