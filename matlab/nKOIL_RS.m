function [Model, AUC, Accuracy,err_count, run_time, mistakes, mistakes_idx, SVs, TMs, AUC_list, Acc_list] = nKOIL_RS( id_train, id_test, Y, K, options  )
...
for t = 1:size(id_train,1),
    ...
    if y_t==+1,
        ...
        [Ap,Bp,flag,ridx] = nUpdateBudgetRS(a(t),id,Ap,Bp,Np,p);
        if(flag==1)
            if(ridx~=-1)
                App(ridx)=a(t);
            else 
        ...
        [An,Bn,flag,ridx] = nUpdateBudgetRS(a(t),id,An,Bn,Nn,n);
        if(flag==1)
            if(ridx~=-1)
                Ann(ridx)=a(t);
            else 
